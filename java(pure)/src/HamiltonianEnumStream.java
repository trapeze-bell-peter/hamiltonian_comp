...
public class HamiltonianEnumStream {
    ...
    public static void main(String[] args) {
        HamiltonianEnumStream h = new HamiltonianEnumStream();
        for (int i = 0; i < 30; ++i) {
        ...
        System.out.println(h.findHamiltonian(st).orElse(Collections.emptyList()));
    ...
    private Optional<Collection<String>> findHamiltonian(String start) {
    ...
    private Optional<Collection<String>> findHamiltoniamRecursively(State current, SimpleLinkedList list, Set<State> visited) {
        SimpleLinkedList ll = new SimpleLinkedList(current, list);
        visited.add(current);
        if (ll.size == journeySize) {
            return Optional.of(collect(ll));
        } else {
            Optional<Collection<String>> any = graph.get(current).stream()
                    .filter(s -> !visited.contains(s))
                    .map(s -> findHamiltoniamRecursively(s, ll, visited))
